...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateCaptionQuery = "UPDATE filter SET
    name = '$name',
    brand = '$brand',
    weight = '$weight',
    ...
    if (!empty($_FILES['new_images']['tmp_name'][0])) {
        ...
        $fetchImageQuery = "SELECT images FROM filter WHERE id = '$imageId'";
        ...
        if ($result->num_rows > 0) {
            ...
            foreach ($oldImageFilenames as $oldImage) {
                unlink("../gallery/filter/" . trim($oldImage));
            }
            ...
            foreach ($_FILES['new_images']['tmp_name'] as $key => $tmp_name) {
                ...
                move_uploaded_file($_FILES['new_images']['tmp_name'][$key], "../gallery/filter/" . $newfilename1);
            ...
            $updateImageQuery = "UPDATE filter SET images = '$newImageFilenames' WHERE id = '$imageId'";
